...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        ...
        // 移動するX軸の距離を計算
        let movingDistanceX = CGFloat(self.frame.size.width * 1.3)

        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x: -movingDistanceX, y: 0, duration:4)
        ...
        let createItemAnimation = SKAction.run({
            ...
            // アイテムのY軸をランダムにする
            let random_y = CGFloat.random(in: self.frame.size.height * 0.4..<self.frame.size.height * 0.7)

            itemSprite.position = CGPoint(x: self.frame.size.width, y: random_y)
